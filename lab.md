...
### ❌ Error: secret "mysql-service-secret" not found
This usually happens if the resource names are inconsistent. 
1.  **Check `fullnameOverride`**: Ensure every `values.yaml` has `fullnameOverride` set (e.g., `fullnameOverride: "mysql-service"`).
2.  **Hardcoded References**: If you reference a secret in the `env` section, it must match the name generated by Helm (`<fullnameOverride>-secret`).

### ❌ Pod in CrashLoopBackOff
1.  **Check logs**: `kubectl logs -l app=login-service -n enterprise-lab`
2.  **DB Connection**: Ensure `MYSQL_HOST` is set to `mysql-service` (not `mysql-svc`).
3.  **Liveness Probes**: If pods are restarting frequently, check if the `/health` endpoint is reachable.
...
| **Service** | Load Balancing | Provides internal DNS names like `login-service` which the Frontend uses for API calls. |
